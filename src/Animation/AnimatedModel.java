...
// TODO give each Animated model a Animation ArrayList to hold its possible animations
...
public class AnimatedModel {
	...
	// skin
	private final AnimationVAO model;
	 ...
	 * 
	 */
	public AnimatedModel(AnimationVAO model, Texture texture, Joint rootJoint, int jointCount) {
		this.model = model;
	...
	private void addJointsToArray(Joint headJoint, Matrix4f[] jointMatrices) {
		...
		for (int i = 0; i < headJoint.children.size(); i++) {
			addJointsToArray(headJoint.children.get(i), jointMatrices);
		}
	}
}
