 ...
 * current animation time). The Animator then updates the transforms of all the
...
public class Animator {
	...
	/**
	 * @param entity
	 *            - the entity which will be animated by this animator.
	 */
	...
	private void increaseAnimationTime() {
		animationTime += DisplayManager.getFrameTimeInSeconds();
	...
	private void applyPoseToJoints(Map<String, Matrix4f> currentPose, Joint joint, Matrix4f parentTransform) {
		...
		for (int i = 0; i < joint.children.size(); i++) {
			applyPoseToJoints(currentPose, joint.children.get(i), currentTransform);
		}
