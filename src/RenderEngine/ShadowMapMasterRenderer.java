...
public class ShadowMapMasterRenderer {
	 ...
	 * 
	 * @param inanimateEntities
	 *            - the lists of inanimate entities to be rendered. Each list is
	 *            associated with the {@link TexturedModel} that all the
	 *            entities in that list use.
	 * @param animatedEntities
	 *            - the lists of animated entities to be rendered
	 * @param sun
	 *            - the light acting as the sun in the scene.
	 */
	public void render(Map<TexturedModel, List<InanimateEntity>> inanimateEntities, List<AnimatedEntity> animatedEntities, Light sun) {
		shadowBox.update();
		...
		entityRenderer.renderInanimateEntities(inanimateEntities);
		entityRenderer.renderAnimatedEntities(animatedEntities);
	 ...
	 * also calculated based on the light's direction and the centre position of
	...
	private void prepare(Vector3f lightDirection, ShadowBox box) {
		...
		updateLightViewMatrix(lightDirection, box.getCentre());
	 ...
	 * light. The light itself has no position, so the "view" matrix is centred
	 * at the centre of the "view cuboid". The created view matrix determines
	 ...
	 * @param centre
	 *            - the centre of the "view cuboid" in world space.
	 */
	private void updateLightViewMatrix(Vector3f direction, Vector3f centre) {
		direction.normalise();
		...
		Matrix4f.translate(centre, lightViewMatrix, lightViewMatrix);
	}
