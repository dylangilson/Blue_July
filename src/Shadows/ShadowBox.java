...
public class ShadowBox {
	...
	private static final float MAX_SHADOW_DISTANCE = 150f; // maximum distance from camera where a shadow will still be visible
	...
	public void update() {
		...
		toFar.scale(MAX_SHADOW_DISTANCE);

		Vector3f toNear = new Vector3f(forwardVector);
		toNear.scale(MasterRenderer.NEAR_PLANE);

		Vector3f centreNear = Vector3f.add(toNear, cam.getPosition(), null);
		Vector3f centreFar = Vector3f.add(toFar, cam.getPosition(), null);
	...
	/**
	 * Calculates the centre of the "view cuboid" in light space first, and then
	 * converts this to world space using the inverse light's view matrix.
	 * 
	 * @return The centre of the "view cuboid" in world space.
	 */
	public Vector3f getCentre() {
		...
		Vector4f centre = new Vector4f(x, y, z, 1);
	 ...
	 * @param centreNear
	 *            - the centre point of the frustum's near plane.
	 * @param centreFar
	 *            - the centre point of the frustum's (possibly adjusted) far
	 *            plane.
	 ...
	 * 
	 * @param startPoint
	 *            - the starting centre point on the view frustum.
	 * @param direction
	...
	private void calculateWidthsAndHeights() {
		farWidth = (float)(MAX_SHADOW_DISTANCE * Math.tan(Math.toRadians(MasterRenderer.FOV)));
	...
	/**
	 * @return The maximum distance from the camera to which a shadow will still be visible
	 */
